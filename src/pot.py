...
def pot_eval(init_score, score, label, q=1e-5, level=0.02):
    ...
    while True:
        try:
            #print(f"[SPOT] Try init with level = {lms:.6f} (attempt #{count})")
        ...
        #except: lms = lms * 0.999
            ...
            count += 1
            if count > 100:
                raise RuntimeError(f"SPOT failed to initialize after {count} attempts. Last error: {e}")
        else: break
...
'''def pot_eval(init_score, score, label, q=1e-3, level=0.95):
    """
    init_score: 训练集 anomaly score（1D）
    score: 测试集 anomaly score（1D）
    q: SPOT 风险水平，越大 -> 阈值越低 -> recall 越高（建议 1e-3 ~ 1e-2）
    level: 初始化阈值分位，越小 -> 阈值越低 -> recall 越高（建议 0.90 ~ 0.98）
    """
    ...
    for _ in range(max_retry):
        try:
            ...
            # 关键：用 SPOT 的 extreme_quantile（训练驱动），不要用 test percentile
            pot_th = float(s.extreme_quantile)
            break
        except Exception:
            lms *= 0.995
    else:
        # fallback：只用训练分数做阈值（别用 test）
        pot_th = float(np.percentile(init_score, 100 * level))
